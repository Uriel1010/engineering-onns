...
if __name__ == "__main__":
    ...
    for i in range(100):
        print(f'test #{i}:')
        y, predicted_index = testAcc(model, random.randint(0, len(X_test)))
        if y == predicted_index:
            num_true += 1
        else:
            num_false += 1

    plt.pie([num_true, num_false], labels=['True', 'False'], colors=['green', 'red'], autopct='%1.1f%%', startangle=90)
    # Add a title
    ...
    # in the plot there is 70% success approx
